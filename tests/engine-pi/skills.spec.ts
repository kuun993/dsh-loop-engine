/**
 * Unit tests for the PiSkillProvider: AGENTS.md discovery from the project
...
describe('PiSkillProvider.list', () => {
  ...
  it('discovers the user AGENTS.md from the home directory', async () => {
    await writeFile(join(home, '.pi', 'AGENTS.md'), '# User instructions')
    const provider = new PiSkillProvider(control())
    const candidates = await provider.list({ cwd: project } satisfies SkillLookupOptions)
    expect(candidates).toHaveLength(1)
