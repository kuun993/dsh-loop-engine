/**
 * Factory and lifecycle tests for the Kimi loop plugin: registration, HMR-safe
...
/** Reentrantly invoke the `agentLoopKimi.lifecycle(...)` effect. */
...
describe('createAgent options', () => {
  ...
  it('rolls back a mid-setup lifecycle when the owner fiber unloads', async () => {
    ...
    try {
      ...
      await new Promise<void>((resolve) => { setImmediate(resolve) })
      disposeLifecycleEffect(ctx, 'agentLoopKimi.lifecycle(mid-setup)')
...
describe('resume', () => {
  ...
  it('rejects a resume when the loop deactivates after load', async () => {
    ...
    try {
      ...
      const ctx = await persistentHarness(root)
      try {
        const loop = ctx.agentLoopKimi as unknown as {
