...
- **托管引擎下选 dsh 的真实模型会交给该引擎**：四个驱动**每个 step 都重读**这条会话当前的选择（`sessionModelOverrideOf`，`src/driver-core/session-model.ts`），选择指着一条真实 dsh 模型（provider **不是**托管路由标签，也不是旧四家）时**原样透传**给引擎——pi 走 `--model <provider>/<model>`、claude-code 走 `Options.model`、codex 走 `thread/start` / `turn/start` 的 `model`、kimi 走 ACP `session/set_model`。能不能用取决于引擎自己的凭据与 provider 配置，**引擎拒绝就报错**（kimi 的 `set_model` 回一个错误帧即让那一步失败），插件不替它吞掉、也不静默退回原生默认。选择是共享标签 `external/default`（菜单里那条 `default`，即"交回引擎自己决定"）或日志里**根本没有任何选择**时，**一个模型参数都不下发**：用引擎自己的原生默认，或部署在 composition 里钉住的 `config.model`（**会话选择优先，pin 只是回落**）。所以那一次选择**不再**只改部署默认模型（见下）——它也改变了本会话引擎的模型。
