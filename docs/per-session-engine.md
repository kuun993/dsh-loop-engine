...
规则定义在 `src/agent-preset-ids.ts:64`（engine → preset id，`enginePresetId`）与 `src/agent-preset-ids.ts:86`（preset id → engine，`engineOfPreset`）；只有本插件 own 的 id 才算引擎，任何部署自己写的 preset 一律落到 in-process。旧版单 preset id 是具名常量 `LEGACY_HOSTED_PRESET_ID`（`src/agent-preset-ids.ts:55`）。这个模块**零导入**，所以 node 侧与浏览器侧共用同一份规则：`src/preset.ts:43-47`、`src/settings.ts:22-27` 只是把它转出去（理由见 `docs/architecture.md` §4.5）。

三态（同一个类型 `SessionEngine`，`src/agent-preset-ids.ts:103-108`）的含义与显示规则：
...
判定函数是 `sessionEngineOf`（`src/agent-preset-ids.ts:264`）。`hostedEngineOf`（`src/agent-preset-ids.ts:281`）是路由那一半：它把三态收成「托管引擎 id 或 undefined」，所以 `legacy` / `unset` / `in-process` 三种回答在路由上都落回 harness loop。
...
形状定义在 `src/agent-preset-ids.ts:125-135`（`SessionEngineReport`），判定在 `src/engine-of-session.ts:128`（`engineReportOfSession`），端点由 `RouterLoop.reportEngine`（`src/router-loop.ts:336`）供货（没有挂上路由器时退回 `engineOfSession`）。

- **判据是「谁在驱动」，不是「写了什么」。** 路由器 `live` 记账里的 `engine`（`RouterLoop.live`，`src/router-loop.ts:174`）是它**自己构建**这个 agent 时用的引擎——它最清楚这条会话现在由什么在跑，所以它压过记录。
...
**turn-status 行与 chip 同源，只跟在屏的那条会话，而且只画「实际」**：对话页那行「深度求索中…」的配色与字形跟着**当前会话**的引擎走，不跟设置里的默认值，也不跟 pending——一次还没落地的切换不该改变这行动画。它由一个 **document 级属性** `<html data-loop-engine>` 驱动（样式表要按属性选择器选中 harness 哈希过的类名，而这一行不是 slot、插件也改不了它的文本），而属性只能由**在屏会话**写：chip 与 composer 共用的 hook `useEngineOfSession`（`src/client/use-session-engine.ts:71-101`）先声明焦点（`focusTurnStatusSession`，`src/client/turn-status.ts:287`）、再按同一个会话 id 反射**报告里的 `engine`**（`hostedEngineOf(report.engine)`，`reflectTurnStatusEngine`，`:265`，守卫在函数第一行：**非焦点会话的反射是无操作**；卸载时 `blurTurnStatusSession`，`:303`），样式表 `installTurnStatusStyles` 只负责注入（`src/client/turn-status.ts:340`）。所以 chip 显示 Pi 时那一行画的就是 Pi 的动画，而且**不会被别的会话改掉**：缓存 `SessionEngineCache` 发布答案时只通知该会话的 watcher（`src/client/session-engine.ts:704-706` 的 `publish`，**不再碰 document**）——旧实现是在 publish 里无条件反射，于是不在屏的会话（或切换会话时旧会话那次迟到的取值）能把它改成自己的引擎，用户实测到的正是"屏上跑 Pi、那一行画出 Kimi 的月亮"。`legacy` / `unset` / 宿主还没答出来 / 换引擎后缓存被作废的那一刻都**不画**（清掉属性、恢复 harness 原样）——不知道是哪个引擎就不冒充任何一个。属性不在卸载时清除：同一会话的另一个界面还在用它，而残留属性在没有这一行的页面上无害。
...
- **preset 选择器同时是对引擎的一次选择——只在"这个会话已有插件记录"的空白会话上**：用 harness 自己的选择器换 preset 时，如果新 preset 映射到**别的**引擎、而这个会话已经有插件记录，插件会把记录一并更新到那个引擎（"最后一次用户动作胜出"，`src/router-loop.ts:580-605`）。没有记录的会话不进记录，引擎继续由 preset 映射回答。
...
- **模型选择与记录同时落盘**：切回 **in-process** 时，插件在写下记录之后**立刻**把这条会话的模型选择改成**部署默认模型**（§5.2「切回 in-process 时重置模型选择」）。原因是会话的选择此时正指着引擎的 provider 标签（`kimi`/`claude-code`/…），而那个标签只有本插件的**占位路由**服务、真有模型调用过来会直接抛 `HOSTED_ENGINE_ROUTE`——进程内那一侧才是真去调模型的。
- **进行中的输出永远不会被中断**：正在跑的时候直接拒绝，不做抢占（§5.3）。
- **空白会话仍然照旧**：宿主自己的 preset 选择器对没跑过一轮的会话依然可用（`AgentPresets.select`，主仓 `packages/preset/agent-presets/src/index.ts:709-726`）。换 preset 时 harness 只重组合 agent 的 scope、**不重建 agent**，所以插件自己补一步：若新 preset 指向**别的**引擎且该会话是空白的，就释放旧 agent，下一次 resolve 用新引擎重建（`src/router-loop.ts:580-605`）。这条路径也走 release（会话被拆掉重建），理由见 §5.1 末尾；若新 preset 映射回 **in-process**，它还会在释放之前做上面那条同样的模型选择重置（§5.2「切回 in-process 时重置模型选择」）。
  - "空白"= 没有开着的 turn 且 `lastTurn === 0`；**只跑过斜杠命令不算跑过一轮**（主仓 `packages/preset/agent-presets/src/index.ts:712-713`）。
  - 这条路径同时实现"最后一次用户动作胜出"：会话已有插件记录时，把记录也更新到新 preset 指向的引擎，否则两个入口会互相打脸（选择器按 preset 重建，下一次 resume 又按记录搬回去，`src/router-loop.ts:580-605`）。
- **resume（宿主还没有活 agent 时的构建路径：进程重启后的冷读、从列表里打开一条本进程没加载过的会话、或一条刚被切换释放掉的会话）按记录优先**：有记录用记录，没记录用日志里记的 preset（`src/router-loop.ts:306-317`，读走 `src/engine-of-session.ts:78-92`）。所以"关掉浏览器明天再来"不会让你的 Kimi 会话变成 in-process，也不会丢掉换引擎的结果。**涉及 in-process 的那次切换就靠这条路落地**：会话已经是冷的，页面重载并重新打开它的那一次 resolve 会按记录构建它（§5.2）。

### 5.1 托管引擎之间：原地换手（hot swap）

**会话不会被拆掉。** 一次换引擎（`src/router-loop.ts:378-415` 的 `selectEngine`，换手在 `:482-517` 的 `hotSwap`）按固定顺序做这些事：
...
3. **切回 in-process 时重置模型选择**（`src/router-loop.ts:406-412`）：要换成的引擎是 `in-process` 就把这条会话的模型选择写成部署默认（§5.2「切回 in-process 时重置模型选择」）。目标是托管引擎时**不写**。
4. 记录与活 agent 的引擎相同时返回（记下就完了，不重建，也不重载）。
5. 否则按 §5.2 的判据分两路：**任一边是 in-process** → `release`（`:448-453` 的 `move` 调 `:546-551` 的 `release`），回包带 `reload: true`；**两边都是托管引擎** → `hotSwap`：
   ...
   3. 路由器把 `live` 记账换成新 agent（`src/router-loop.ts:242-261` 的 `adopt`）。
...
- **下一轮就是新引擎**：宿主的 prompt 路径每一轮都按 sessionId 现取 agent（主仓 `packages/api/session-controller/src/agent.ts:187` 的 `liveAgent`），所以注册完继任者，下一条输入自然落到它身上。
- **换手失败怎么办**：旧机器已经退场、继任者没能发布，这时**没有东西在驱动这条会话**。插件把这条会话的 `SessionLifetime` 释放掉（关写句柄 + 离开 store，`src/router-loop.ts:508-514`），于是它变回一条冷会话：下一次打开时按记录（也就是刚写下的新引擎）重建，和任何宿主没加载过的会话一样。拒绝原因里写清了失败原文。
...
1. **记录立即写下**（`selectEngine`，`src/router-loop.ts:378-415`）；
2. **切回 in-process 时还有第 2 件立刻发生的事：这条会话的模型选择被写回部署默认**（见下面那一段）；
3. **释放这条会话的 agent**（`release`，`src/router-loop.ts:549-554`）。走的就是 harness 自己那套 teardown（`cancel → whenIdle → scope.dispose → handle.close() → detachAgent() → detachSession()`，主仓 `packages/core/session/src/index.ts:1064-1074` 的 `detachEntered`）——**会话因此变冷，而记录就是它下一次构建要用的引擎**；
4. **成功回包带 `reload: true`**（形状 `LoopEngineSelectResult`，`src/agent-preset-ids.ts:201-243`）：`{ ok: true, engine: '<新引擎>', reload: true }`；
...
**切回 in-process 时重置模型选择**（0.1.5-rc3 起）：这条会话的模型选择**跟着引擎走**：四个托管引擎都会把自己的 provider 标签（`kimi` / `claude-code` / `codex` / `pi`）写进会话的 `request/header`，而宿主是从最新一条 header 推导这条会话的模型选择的。切回 in-process 时这就有问题了——这一侧**真的**去调模型，而那条选择只是一个**占位路由**服务的标签：占位路由平时不报错（它的作用就是让宿主"这个 provider 没人服务"的检查放行，否则托管会话的第二轮就失败），可真被调用时它 loud 抛 `HOSTED_ENGINE_ROUTE`。用户实测到的那句"托管引擎之间怎么切都正常、切回 harness 默认每轮报错"就是这条。所以插件把选择写回**部署默认模型**（`agentDefaultModel.currentSelection()`，宿主给新会话用的那个），写的是 harness 自己的 `model/selection` 事件——与宿主模型选择器写下的是**同一个事件、同一个形状**（`{ provider, model, reasoningEffort? }`），因此折进的是宿主读的那一格投影，宿主下一次给这条会话装选择时读到的就是它（写入 `src/model-selection-reset.ts:111-121`，判据 `:123-158`）：

- **立刻写，不等引擎**：引擎要等这次重载带来的那次构建，模型选择则在**切换那一刻**就落盘——它就是"下一次构建"要读的东西，早写没有副作用。这段时间里活 agent 仍是托管引擎，它自己那轮用的是引擎原生的模型，这条选择对它没有意义。
- **幂等**：会话当前选择已经是部署默认（provider + model 相同）就不再追加，反复切 in-process 只在日志里留一条（判据先读投影的 `pending`，没有就读会话自己最新的 `request/header`）。
- **反向不写**：切到托管引擎（含托管引擎之间）**不碰**模型选择——那台引擎会写自己的标签进下一条 header；而唯一读这条选择的 pi 驱动只认自己目录里的模型，别家的默认值它直接丢掉。
- **拿不到默认值就跳过**：composition 里没有 `agentDefaultModel` 服务、或它答不出可用的 provider/model → 只报**一次** warn、不写事件，**切换本身照常成功**（`src/model-selection-reset.ts:160-203`）。
- **preset 通道同样**：宿主自己的 preset 选择器把一条**空白**会话换回映射为 in-process 的 preset 时，插件在释放旧 agent **之前**做同一件事（`src/router-loop.ts:602`）。
...
- **"撤回"只对释放失败那次有意义**：正常路径上会话已经变冷、页面已经重载，记录就是它跑的引擎——没有"当前引擎"可以对回了。释放失败时报告的 `engine` 仍是旧引擎，选中它就把记录改回去（宿主比对的是活 agent 的引擎，`src/router-loop.ts:413`），什么都不重建。
...
`remote.loopEngine.select` 的返回形状是 `{ ok: true, engine, reload? } | { ok: false, code, reason }`（`src/agent-preset-ids.ts:201-243`）。`code` 是**原因码**（`LoopEngineRefusalCode`，`src/agent-preset-ids.ts:165-190`），决定界面说哪句话；`reason` 是**宿主自己那句英文完整句**，不再是正文，而是详情（两个带底层错误的码会以小字显示它，其余情况只在码不认识时才拿出来当正文）。客户端侧的映射在 `refusalFace`（`src/client/locales.ts:299-301`，表 `:278-286`）。
...
拒绝一律**不改**会话的引擎：记录没写、agent 没动，路由与显示仍然一致（Remote 会把**同一个**引擎报成 `engine`，没有第二个字段）。端点的实现在 `src/engine-remote.ts:233-255`，判定在 `src/router-loop.ts:378-415`。上面表格最后那条**不是**拒绝——记录已经写下、会话的 agent 也已经被释放，客户端据此重载页面并回到这条会话（§5.2）；这一行里的**切回 `in-process`** 那一半还会顺手把这条会话的模型选择改成部署默认模型（§5.2「切回 in-process 时重置模型选择」），**切到托管引擎**那一半不碰它。另外：拿不到部署默认模型时插件只 warn，**不会**因此拒绝这次切换。
...
- **多引擎同进程并存**：进程里只有**一个** AgentFactory（harness 的硬约束），但它是路由器；每个用到的引擎在首次使用时构造一个 runtime，之后把该引擎的每个会话交给它（`src/router-loop.ts:223-229`）。所以"会话 A 跑 Codex、会话 B 跑 Kimi"是真正的并发，不是排队。
...
- **模型选择**：四个引擎都原生持有模型，页面上的模型选择对它们不生效；默认引擎是 Claude Code 时设置页会明确提示这一点（`src/client/LoopEngineSection.tsx:212`）。**切回 `in-process` 时插件会把这条会话的模型选择写回部署默认模型**（§5.2）：那条选择此前是引擎的 provider 标签，而进程内这一侧要真调模型，标签只有占位路由服务、真调用会抛 `HOSTED_ENGINE_ROUTE`。
- **权限与沙箱**：按会话的 dsh permission 设置折叠进各引擎自己的权限面，细节见各 `docs/engine-*.md`。
- **provider 标签常驻**：四个引擎的 provider 标签在同一进程里同时注册为占位路由（`src/provider-route.ts:27-32`），所以每个寄宿会话的第二轮 prompt 都不会因为"没有 adapter 服务这个 provider"被拒（宿主侧的拒绝点：主仓 `packages/api/session-controller/src/commands.ts:323-328`）。细节见 `docs/architecture.md` §3.6。所以原地换手之后的**第一轮**也不会被宿主以"provider 没有 adapter"为由拒绝：四个标签一直都在。
...
- ⚠️ **旧会话的引擎只能如实说"不知道"**：旧版本只有一个 preset id `loop-engine`（没有引擎后缀），而新规则只认 `loop-engine-<engine>`（`src/agent-preset-ids.ts:86`），所以日志里记着 `loop-engine` 的旧会话匹配不到任何引擎。路由侧因此继续按 in-process 跑它（§1.2 的兜底），显示侧也**不冒充**当年那个托管引擎：这条会话**没有活 agent** 时（冷会话）会话头 chip 与 composer 显示 **「旧版托管引擎」**（判定 `src/agent-preset-ids.ts:264`，文案 `src/client/locales.ts:103` / `:148`）。它**已经打开**时 chip 报的就是 `in-process`——那是它此刻真正在跑的东西（§1.3 末条）。同理，**没有记录 preset 的会话**也不声称任何引擎：chip 不渲染，composer 显示「未记录」（`:104` / `:149`）。想在旧会话上直接换成某个引擎，用 composer 选择器（§4.2/§5）即可——不需要新建会话。磁盘上遗留的 `$DSH_HOME/.agent-presets/loop-engine/` 不会被删除（插件只写自己那四份，不清扫别人的目录），它仍会出现在 preset 菜单里，但选中它等价于 in-process。
...
| 换回**当前实际在跑**的那个引擎，什么都没重建、也没报重载 | 这是设计：宿主比对的是活 agent 的引擎（`src/router-loop.ts:413`）。只有在"记录里的引擎没能接管"那种罕见情况下才谈得上"撤回"：选中实际在跑的那个，记录就跟着活 agent 改回去，标注随即消失。 |
...
| 某条会话第二轮 prompt 报 `model-unavailable` | 该会话的 provider 标签没有被任何 adapter 服务——正常部署下不会发生（四个标签常驻，§6）。若真遇到，先确认插件在 profile 里被组合、且没有卸载。 |
