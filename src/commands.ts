...
export interface CommandInvocation {
  ...
  readonly agent: { readonly followup: (input: UserMessage) => void }
 ...
 */
export function forwardClaudeCodeCommand(name: string): (invocation: CommandInvocation) => CommandResult {
...
export const CLAUDE_CODE_COMMANDS: readonly CommandDefinition[] = [
 ...
 */
function commandDescription(raw: string): string | undefined {
  ...
  if (trimmed.startsWith('---\n')) {
    ...
    if (closing <= 0) return undefined // dangling `---` opener — nothing to describe
