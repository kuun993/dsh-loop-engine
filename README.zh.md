...
四个引擎**共用一个** provider 标签(`external`),在 llm registry 里注册为一条占位路由(`src/provider-route.ts`):每个引擎把**同一个**标签写进所属会话的 `request/header`,而 Web 宿主会拒绝 provider 没有适配器服务的回合。**为什么不是四个**:浏览器模型目录是整个 Host 代际共享、不按会话区分的(主仓 `packages/api/session-controller/src/catalog.ts`),四个引擎各占一个标签会让每条会话的菜单里出现四条一模一样的 `default`。这条占位路由**只广告一个**模型条目——`{ provider: 'external', id: 'default', name: 'default' }`(分组显示名取 `providerInfo().name`,与 wire 上的 id 是同一个 ASCII 串 `external`)——而这个 `default` 也正是部署没钉模型时引擎写进 header 的模型标签:模型菜单按 `model.id` 解析会话的 `(provider, model)`,两边必须是同一个串,那一格才会显示 `default`,而不是拼出一个没有任何适配器能服务的复合串。早期版本逐引擎写的四家标签(`claude-code`/`codex`/`pi`/`kimi`)不再注册,但仍被 `isHostedProviderRoute` 判为托管路由,好让带旧标签的老会话被重置成 `external/default`(或部署默认)。**会话自己选的真实 dsh 模型会被交给引擎**:四个驱动每个 step 都重读会话选择(`src/driver-core/session-model.ts`),把真实模型按各引擎自己的接口下发——Pi 的 `--model <provider>/<model>`、Claude Code 的 `Options.model`、Codex 的 `thread/start` 的 `model`、Kimi 的 ACP `session/set_model`;用的是引擎自己的凭据与 provider 配置,所以能不能用取决于引擎,引擎拒绝就报错。选 `external/default`(或日志里一条选择都没有)时一个模型参数都不下发,用引擎原生默认或部署钉住的 `config.model`(会话选择优先,pin 只是回落)。插件同时给 harness loop 驱动的会话写回一条真实选择(`src/model-selection-reset.ts`)。除这些 `default` 条目外,模型目录其余部分不变。
...
- **托管引擎下选一个模型会被交给该引擎。** 默认用哪个模型由引擎自己原生决定;那个会话里没有 dsh 的 `/plan`、`/compact`、`/goal`、goal 工具与 dsh skill 目录(`src/preset.ts:79`)。模型菜单里所有托管引擎共用一个 `external` 分组,名下只有一条 `default` 条目(`src/provider-route.ts`),意思是「交回引擎自己决定」。但在这里选一条**真实 dsh 模型**并非不生效:四个驱动每个 step 都重读会话选择(`src/driver-core/session-model.ts`),把模型按各引擎自己的接口下发——Pi 的 `--model <provider>/<model>`、Claude Code 的 `Options.model`、Codex 的 `thread/start` 的 `model`、Kimi 的 ACP `session/set_model`。它用引擎自己的凭据与 provider 配置,所以能不能用取决于引擎,引擎拒绝就报错、而不是静默退回原生默认。选 `external/default`(或没有任何选择)时不下发任何模型参数,用引擎原生默认或部署钉住的 `config.model`(会话选择优先,pin 回落)。另一件值得知道的事:宿主把这个选择存成**部署默认**,所以插件给 harness loop 驱动的会话写回一条真实选择(`ModelSelectionReset`——切回 `in-process` 时 `resetFor`、构建这类会话时 `guardFor`,`src/model-selection-reset.ts:148-158`、`:172-175`;用户可见语义见 `docs/per-session-engine.md` §5.2)。插件不做逐引擎目录校验:Pi 的 `pi --list-models` 探针保持删除,引擎自己报它服务不了哪个模型。
